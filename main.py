...
api_id = 10754737
api_hash = '39a7fc5d9bd640256b95eafef6fd768b'
#api_id = 7297355
...
async def main():
    ...
    # print(message_1337)
    #last_message_id = "4605"
    last_message_id = "5225"
    messagesList = ''
    ...
    print("filterstrpost:", filterStrPost)
    ...
    while True:
        ...
        #limit=3, min_id=int(last_message_id))
        #async for message in client.iter_messages(message_channel_id, limit=3, min_id=int(last_message_id)):
        # async for message in client.iter_messages(message_channel_id, limit=1, min_id=int(last_message_id), 
        #     search=filterStr):
            ...
            if len(msgStrSplitFirst) > 1:
                ...
                if len(msgStrSplit) > 0:
                    ...
                    if len(tickerSymbol) <= 5 and len(tickerSymbol) > 0:
                        ...
                        # "last shares of "
                        # "target 
                        msgStrFindPriceEntry = msgStr.split(filterStrPost + tickerSymbol + " at ")
                        if len(msgStrFindPriceEntry) > 1:
                            msgStrFindPriceEntrySpace = msgStrFindPriceEntry[1].split(' ')
                            if len(msgStrFindPriceEntrySpace) > 0:
                                buyInPriceStr = msgStrFindPriceEntrySpace[0]
                                targetPriceFinal = 0
                                getReadyToBuy(upperCaseSymbol, buyInPriceStr, targetPriceFinal)
                                #Reset Var
                                #targetPriceStrSplit = "undefined"
                                targetPriceStrSplit = msgStrFindPriceEntry[1].split('target ')
                                if len(targetPriceStrSplit) > 1:
                                    ...
                                    if len(targetPriceStrSplitSpace) > 0:
                                        ...
                                        print(targetPriceFinalStr)
                                        
                                        


                            # #<< Attempting float check on buyin and target >>#
                            # #ordersent = False
                            #     try:
                            #         print("Checking try...")
                            #         if isfloat(buyInPriceStr) and isfloat(targetPriceFinalStr):
                            #             print("Sending getreadybuy order: ",upperCaseSymbol, buyInPriceStr, targetPriceFinalStr)     
                            #             getReadyToBuy(upperCaseSymbol, buyInPriceStr, targetPriceFinalStr)
                            #             #ordersent=True
                            #         elif isfloat(buyInPriceStr):
                            #             print("Can't resolve targetPriceFinalStr, default to fallback")
                            #             targetPriceFinal = "undefined"
                            #             getReadyToBuy(upperCaseSymbol, buyInPriceStr, targetPriceFinal)
                            #             #ordersent = True
                            #     except:
                            #             print("Couldn't resolve targetPriceFinalStr, defaulting to 3% profit taker")
                            #             if isfloat(buyInPriceStr):
                            #                 targetPriceFinal = "undefined"
                            #                 getReadyToBuy(upperCaseSymbol, buyInPriceStr, targetPriceFinal)
                            #                 #ordersent = True  
                        # try:
                        ...
                        #             float(buyInPriceStr)
                        #             try:
                        #                 float(targetPriceFinalStr)        
                        #                 #getReadyToBuy(upperCaseSymbol, buyInPriceStr, targetPriceFinalStr)
                        #                 buyFulfill = "Success"
                        #             except:
                        #                 print("target price fail to cast as float")
                        #         except:
                        #             print("price fail to cast to float")
                        #         print("Order fulfill status:", buyFulfill)
                        ...
                        #         if len(msgStrFindPriceEntrySpace) > 1:
                        #             buyInPriceStrSecond = msgStrFindPriceEntrySpace[1]
                        #             if isfloat(buyInPriceStrSecond):
                        #                 buyInPriceStr = buyInPriceStrSecond
                        #         if isfloat(buyInPriceStrFirst):
                        #             buyInPriceStr = buyInPriceStrFirst
                                
                        #         print(buyInPriceStrFirst)
                        #         print(buyInPriceStrSecond)
                        ...
                        #                 print("targetPriceFinalStr", targetPriceFinalStr)
                        #                 if buyInPriceStr:
                        #                     print("Price Found: ", buyInPriceStr, targetPriceFinalStr)
                        #                     getReadyToBuy(upperCaseSymbol, buyInPriceStr, targetPriceFinalStr)
